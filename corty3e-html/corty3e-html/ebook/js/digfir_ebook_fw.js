...
xBookUtils.DFVersionNumber = "20160519-001";
...
var Player_manuscript_type = Player.extend({
    ...
    */
	...
	// helper function for scrolling to anchor ID, will be used below
	function scroll_to_anchor_id() {
	    
	    // If we don't have and anchor then we are done
	    if (xBookUtils.emptyValue(anchor)) {
		return;
	    }
	    
	    xBookUtils.debug("we have an anchor: " + anchor);
		...
		if ($anchor.length > 0) {
		    ...
		    var anchor_y = Math.round($anchor.offset().top) - 100;
		    
		    xBookUtils.debug("found anchor, scrolling to ID " + anchor);
		    $scrollingWindow.scrollTop(anchor_y);
		    //$scrollingWindow.scrollTo($anchor, 100, {axis:'y'});
		    
		    // we need to wait for all images to load since our images
		    // don't have heights set on them
		    $('#manuscript').imagesLoaded(function() {
			xBookUtils.debug("scroll_to_anchor_id: scrolling to anchor " + anchor + " after imagesLoaded");
			anchor_y = Math.round($anchor.offset().top) - 100;
			$scrollingWindow.scrollTop(anchor_y);
			//$scrollingWindow.scrollTo($anchor, 100, {axis:'y'});	
		    });
		}
	...
	if (($parentIFrame.length > 0) && ($scrollingWindow.length > 0)) {
		...
		if (scrollTo < $parentIFrame.height()) {
		    xBookUtils.debug("iframe height is tall enough, scrolling to " + scrollTo);
	...
	/* We don't need this any more because we are scrolling to anchors above.
...
/* Returns true if the page is being viewed by an instructor.  
   Otherwise returns false. */
